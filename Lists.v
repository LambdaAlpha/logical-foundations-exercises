...
match l with
|nil => nil
|x :: tl => if oddb x then x :: oddmembers tl else oddmembers tl
end.
...
Proof.
intros l1 l2 l3 l4.
assert (l2 ++ l3 ++ l4 = (l2 ++ l3) ++ l4) as H.
{ rewrite app_assoc. reflexivity. }
rewrite H.
assert (forall l:natlist, l1 ++ l ++ l4 = (l1 ++ l) ++ l4) as H1.
{ intros l. rewrite app_assoc. reflexivity. }
rewrite H1.
assert (l1 ++ l2 ++ l3 = (l1 ++ l2) ++ l3) as H2.
{ rewrite app_assoc. reflexivity. }
rewrite H2. 
reflexivity.
Qed.
...
Proof.
intros l1 l2 H.
assert (rev (rev l1) = rev (rev l2)) as H1.
{ rewrite H. reflexivity. }
rewrite rev_involutive in H1. rewrite rev_involutive in H1. rewrite H1. reflexivity.
Qed.
