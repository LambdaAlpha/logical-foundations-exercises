...
|E_BNot (be : bexp) (b : bool) : (bevalR be b) -> bevalR (BNot be) (negb b)
|EAnd (be1 be2 : bexp) (b1 b2 : bool) : (bevalR be1 b1) -> (bevalR be2 b2) -> bevalR (BAnd be1 be2) (andb b1 b2)
.
...
- generalize dependent bv. induction b; 
  try (intros bv H; inversion H; simpl in *; 
    try (rewrite aeval_iff_aevalR' in *); 
    try (apply IHb in H1);
    try (apply IHb1 in H2; apply IHb2 in H4);
  subst; reflexivity).
- generalize dependent bv. induction b;
  intros bv H; simpl in *; subst; constructor; 
  try (rewrite aeval_iff_aevalR' in *);
  auto.
Qed.
(** [] *)
...
|NW_Ass (X : string) (n : aexp) : no_whilesR (X ::= n)
|NW_Seq (c1 c2 : com) : (no_whilesR c1) -> (no_whilesR c2) -> no_whilesR (c1 ;; c2)
|NW_If (b : bexp) (c1 c2 : com) : (no_whilesR c1) -> (no_whilesR c2) -> no_whilesR (TEST b THEN c1 ELSE c2 FI)
.
...
- intros. induction c;
  ...
  (inversion H; apply andb_true_iff in H1; destruct H1; apply IHc1 in H0; apply IHc2 in H1; constructor; assumption))).
- intros. induction c; inversion H; simpl; intuition.
Qed.
(** [] *)
...
Theorem no_whiles_terminating: forall (c : com),
  no_whiles c = true -> forall (st : state), exists st1, st =[ c ]=> st1.
Proof.
induction c.
- intros. exists st. constructor.
- intros. exists (x !-> (aeval st a); st). constructor. reflexivity.
- intros. inversion H. apply andb_true_iff in H1. destruct H1. intuition. destruct H2 with st. destruct H3 with x. exists x0. apply (E_Seq c1 c2 st x x0 H4 H5).
- intros. inversion H. apply andb_true_iff in H1. destruct H1. intuition. destruct H2 with st. destruct H3 with st. destruct (beval st b) eqn:E.
  + exists x. apply (E_IfTrue st x b c1 c2 E H4).
  + exists x0. apply (E_IfFalse st x0 b c1 c2 E H5).
...
|x :: xs => match x with
            ...
            |SLoad x => s_execute st ((st x) :: stack) xs
            |SPlus => match stack with
                      |nil => stack
                      |[m] => stack
                      |m :: n :: ns => s_execute st ((n + m) :: ns) xs
                      end
...
Lemma s_execute_app: forall (st : state) (stack:list nat) (x : sinstr) (xs:list sinstr),
  s_execute st stack (x :: xs) = s_execute st (s_execute st stack [x]) xs.
Proof.
Admitted.

Theorem s_compile_correct_gen : forall (st : state) (e : aexp) (stack:list nat),
