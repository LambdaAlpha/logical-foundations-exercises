...
- intros. induction H.
  + apply even'_0.
  + assert (H1:S (S n) = 2 + n). { simpl. reflexivity. } rewrite H1. apply even'_sum.
    * apply even'_2.
    * apply IHeven.
Qed.
(** [] *)
...
intros. induction H0.
- simpl in H. apply H.
- simpl in H. inversion H. apply IHeven. apply H2.
Qed.
(** [] *)
...
- assert (n + n + (m + p) = (n + m) + (n + p)) as H1. 
  {
    rewrite plus_assoc. rewrite <- (plus_assoc n n m). rewrite (plus_comm n m). rewrite (plus_assoc n m n). rewrite (plus_comm m n). rewrite (plus_assoc (n + m) n p). reflexivity.
  }
  rewrite H1. apply ev_sum.
  + apply H.
  + apply H0.
...
- rewrite plus_comm. simpl. rewrite plus_comm. apply (le_trans a (a + b) (S(a + b))).
  + apply IHb.
  + apply le_S. apply le_n.
Qed.
...
Theorem lt_S : forall n m,
  ...
  n < S m.
Proof.
unfold lt. intros. apply (le_trans (S n) m (S m)).
- apply H.
- apply le_S. apply le_n.
Qed.

Check Datatypes.bool.
...
Proof.
intros. apply leb_correct. apply leb_complete in H. apply leb_complete in H0. apply (le_trans n m o).
- apply H.
- apply H0.
Qed.
(** [] *)
...
- intros. induction H.
  ...
  + simpl in IHR. injection IHR as H1. rewrite plus_comm in H1. simpl in H1. injection H1 as H2. rewrite plus_comm. apply H2.
  + rewrite plus_comm. apply IHR.
- intros. generalize dependent o. generalize dependent n.  induction m.
  + induction n.
    * intros. destruct o.
      -- apply c1.
      -- inversion H.
    * intros. destruct o.
      -- inversion H.
      -- simpl in H. injection H as H1. simpl in IHn. apply IHn in H1. apply c3. apply H1.
  + induction n.
    * intros. destruct o.
      -- inversion H.
      -- simpl in H. rewrite <- plus_n_O in H. injection H as H1. rewrite (plus_n_O m) in H1. apply IHm in H1. apply c2. apply H1.
    * intros. destruct o.
      -- inversion H.
      -- simpl in H. injection H as H. rewrite plus_comm in H. simpl in H. destruct o.
        ++ inversion H.
        ++ injection H as H. rewrite plus_comm in H. apply IHm in H. apply c2. apply c3. apply H.
Qed.
(** [] *)
...
|SNil l :subseq nil l
|SApp x l1 l2 (H:subseq l1 l2) :subseq l1 (x::l2)
|SMatch x l1 l2 (H:subseq l1 l2) :subseq (x::l1) (x::l2).

Lemma cons_app: forall (X : Type) (x : X) (l : list X),
  x :: l = [x] ++ l.
Proof.
...
intros. induction H.
- apply SNil.
- simpl. apply SApp. apply IHsubseq.
- simpl. apply SMatch. apply IHsubseq.
Qed.
...
- intros. induction l.
  + apply H.
  + apply SApp. apply IHl.
- intros. apply SApp. apply IHsubseq. apply H.
- intros. inversion H.
  + apply SNil.
  + apply SApp. apply IHsubseq. apply H3.
  + apply SMatch. apply IHsubseq. apply H3.
Qed.
(** [] *)
...
- generalize dependent s1. induction s2.
  ...
  + intros. destruct s1.
    * inversion H.
    * injection H as H. rewrite H. rewrite H0. simpl. rewrite cons_app. apply MApp.
      -- apply MChar.
...
- intros. induction re.
  ...
  + simpl. reflexivity.
  + simpl. reflexivity.
  + destruct H. simpl. apply andb_true_iff. split.
    * apply IHre1. inversion H. exists s1. apply H3.
    * apply IHre2. inversion H. exists s2. apply H4.
  + simpl. apply orb_true_iff. destruct H. inversion H.
...
- intros. induction re.
  ...
  + inversion H. apply andb_true_iff in H1. destruct H1. apply IHre1 in H0. destruct H0. apply IHre2 in H1. destruct H1. exists (x ++ x0). apply MApp.
    * apply H0.
    * apply H1.
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp),
  ...
  exists ss : list (list T),
    ...
    /\ forall s', In s' ss -> s' =~ re.
Proof.
intros. remember (Star re) as sre. induction H.
- discriminate Heqsre.
- discriminate Heqsre.
- discriminate Heqsre.
- discriminate Heqsre.
- discriminate Heqsre.
- injection Heqsre as H. exists []. split.
  + simpl. reflexivity.
  + intros. simpl in H0. destruct H0.
- injection Heqsre. intros E. destruct IHexp_match2.
  + rewrite E. reflexivity.
  + exists (s1 :: x). destruct H1. split.
    * simpl. rewrite <- H1. reflexivity.
    * intros. destruct H3.
      -- rewrite <- H3. rewrite <- E. apply H.
      -- apply H2. apply H3.
Qed.
(** [] *)
...
(* Lemma le_plus_inv: forall a b c d:nat,
  a + b <= c + d -> (a <= c \/ b <= d).
Proof.
induction a.
- intros. left. apply le_0_n.
- intros. remember H as E. simpl in H. assert (H1:a + b <= c + d). 
  { apply (le_trans (a + b) (S (a + b)) (c + d)).
      - apply le_S. apply le_n.
      - apply H. }
  apply IHa in H1. destruct H1.
    + destruct H0.
      *  *)
...
- simpl. omega.
- simpl. omega.
- simpl. intros. rewrite app_length in H. assert (H1: pumping_constant re1 <= length s1 \/ pumping_constant re2 <= length s2). { omega. } destruct H1.
  + apply IH1 in H0. destruct H0. destruct H0. destruct H0. exists x. exists x0. exists (x1 ++ s2). destruct H0. destruct H1. split.
    * rewrite H0. rewrite <- (app_assoc _ x (x0 ++ x1) s2). rewrite <- (app_assoc _ x0 x1 s2). reflexivity.
    * split.
      -- apply H1.
      -- intros. rewrite (app_assoc _ (napp m x0) x1 s2). rewrite (app_assoc _ x ((napp m x0) ++ x1) s2). apply MApp.
        ++ apply H2.
        ++ apply Hmatch2.
  + apply IH2 in H0. destruct H0. destruct H0. destruct H0. exists (s1 ++ x). exists x0. exists x1. destruct H0. destruct H1. split.
    * rewrite H0. rewrite (app_assoc _ s1 x (x0 ++ x1)). reflexivity.
    * split.
      -- apply H1.
      -- intros. rewrite <- (app_assoc _ s1 x ((napp m x0) ++ x1)). apply MApp.
        ++ apply Hmatch1.
        ++ apply H2.
- simpl. intros. assert (pumping_constant re1 <= length s1) as H1. { omega. } apply IH in H1. destruct H1. destruct H0. destruct H0. destruct H0. destruct H1. exists x. exists x0. exists x1. split.
  + apply H0.
  + split.
    * apply H1.
    * intros. apply MUnionL. apply H2.
- simpl. intros. assert (pumping_constant re2 <= length s2) as H1. { omega. } apply IH in H1. destruct H1. destruct H0. destruct H0. destruct H0. destruct H1. exists x. exists x0. exists x1. split.
  + apply H0.
  + split.
    * apply H1.
...
- simpl. intros. destruct s2.
  + rewrite app_length in H. simpl in H. rewrite <- plus_n_O in H. exists []. exists s1. exists []. split.
    * simpl. reflexivity.
    * split.
      ...
      -- intros. simpl. rewrite app_nil_r. induction m.
        ...
        ++ simpl. apply MStarApp.
          ** apply Hmatch1.
          ** apply IHm.
  + simpl in IH2. destruct IH2.
    * omega.
    * destruct H0. destruct H0. destruct H0. destruct H1. exists (s1 ++ x0). exists x1. exists x2. split.
      -- rewrite H0. rewrite (app_assoc _ s1 x0 (x1 ++ x2)). reflexivity.
      -- split.
        ++ apply H1.
        ++ intros. rewrite <- (app_assoc _ s1 x0 ((napp m x1) ++ x2)). apply MStarApp.
          ** apply Hmatch1.
          ** apply H2.
Qed.
...
Proof.
intros. induction l.
- simpl. Search False. unfold not. intros. destruct H0.
- simpl in H. destruct (eqbP n x).
  + discriminate H.
  + simpl in H. apply IHl in H. simpl. unfold not. intros. destruct H1.
    * unfold not in H0. rewrite H1 in H0. apply H0. reflexivity.
    * apply H. apply H1.
Qed.
(** [] *)
...
|NNil : nostutter nil
|NOne x : nostutter [x]
|NApp x y l (NE:x <> y) (H:nostutter (y :: l)): nostutter (x :: y :: l)
.
...
|INil : InOrderMerge nil nil nil
|IAppFst x l1 l2 l (H : InOrderMerge l1 l2 l) : InOrderMerge (x :: l1) l2 (x :: l)
|IAppSnd x l1 l2 l (H : InOrderMerge l1 l2 l) : InOrderMerge l1 (x :: l2) (x :: l)
.

Theorem filter_in_order_merge: forall (X : Type) (test : X -> bool) (l : list X),
  InOrderMerge (filter test l) (filter (fun x => negb (test x)) l) l.
Proof.
intros. induction l.
- simpl. apply INil.
- simpl. destruct (test x).
  + simpl. apply IAppFst. apply IHl.
  + simpl. apply IAppSnd. apply IHl.
Qed.
(* Do not modify the following line: *)
...
Theorem filter_longest: forall (X : Type) (test : X -> bool) (l sub: list X),
  subseq sub l ->
  (forall x, In x sub -> test x = true) -> 
  (length sub <= length (filter test l)).
Proof.
intros X test l. induction l.
- intros. inversion H. simpl. apply le_n.
- intros. inversion H.
  + apply le_0_n.
  + apply (le_trans (length sub) (length (filter test l)) (length (filter test (x :: l)))).
    * apply IHl.
      -- assumption.
      -- assumption.
    * simpl. destruct (test x).
      ...
      -- apply le_n.
  + rewrite H1 in *. rewrite <- H2 in *. assert (test x = true). { apply H0. simpl. left. reflexivity. } simpl. rewrite H5. simpl. apply le_n_S. apply IHl. 
    * assumption.
