...
- intros. induction l.
  ...
  + destruct H. destruct H. simpl. simpl in H0. destruct H0.
...
- intros. induction l.
  ...
  + simpl in H. destruct H.
    ...
    * simpl. apply IHl in H. destruct H.
      -- left. right. apply H.
      -- right. apply H.
- intros. destruct H.
  + induction l.
...
- unfold combine_odd_even. rewrite E. apply H0. reflexivity.
Qed.
...
Proof.
intros. destruct (oddb n) eqn:E.
- unfold combine_odd_even in H. rewrite E in H. apply H.
- discriminate H0.
Qed.
...
Lemma rev_append_eq: forall (X : Type) (l1 l2 : list X),
...
intros. apply functional_extensionality. intros. unfold tr_rev. induction x.
- reflexivity.
- simpl. apply rev_append_eq.
Qed.
...
- destruct (evenb n) eqn:E.
  + destruct IHn. exists x. rewrite evenb_S. rewrite E. simpl. rewrite <- H. reflexivity.
  + destruct IHn. exists (S x). rewrite evenb_S. rewrite E. simpl. rewrite <- H. reflexivity.
Qed.
(** [] *)
...
- unfold not. intros. destruct (x =? y) eqn:E.
...
- generalize dependent l2. induction l1.
  ...
  + induction l2.
    * simpl. intros. discriminate H0.
    * simpl. intros. apply andb_true_iff in H0. destruct H0. apply IHl1 in H1. apply H in H0. rewrite H0. rewrite H1. reflexivity.
- generalize dependent l2. induction l1.
  + induction l2.
    * intros. reflexivity.
  ...
  + induction l2.
    * intros. discriminate H0.
    * intros. injection H0 as H1 H2. rewrite H1. rewrite H2. simpl. assert (H3: eqb x0 x0 = true). { apply H. reflexivity. } rewrite H3. simpl. rewrite <- H2. apply IHl1. reflexivity.
Qed.
(** [] *)
...
- intros. induction l.
  + simpl. apply I.
  + simpl. simpl in H. apply andb_true_iff in H. destruct H. split.
    * apply H.
    * apply IHl in H0. apply H0.
- intros. induction l.
  + simpl. reflexivity.
  + simpl. simpl in H. destruct H. rewrite H. simpl. apply IHl in H0.  apply H0.
Qed.
...
Proof.
intros. unfold excluded_middle in H. assert (H1: P x \/ ~ P x). { apply H. } destruct H1. 
- apply H1.
...
- intros. assert (H1: P \/ ~ P). { apply H. } destruct H1.
  + apply H1.
  + apply H0 in H1. destruct H1.
- intros. apply H. apply excluded_middle_irrefutable.
Qed.
