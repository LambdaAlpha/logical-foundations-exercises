...
Proof.
  ...
  - destruct IHHce1. destruct IHHce2. exists (S(x + x0)). simpl. 
    assert (x <= x + x0) by (apply le_plus_l). 
    assert (ceval_step st c1 (x + x0) = Some st') by (apply (ceval_step_more x (x + x0) st st' c1 H1 H)). 
    rewrite H2. 
    assert (x0 <= x + x0) by (apply le_plus_r). 
    apply (ceval_step_more x0 (x + x0) st' st'' c2 H3 H0).
  - destruct IHHce. exists (S x). simpl. rewrite H. assumption.
  - destruct IHHce. exists (S x). simpl. rewrite H. assumption.
  - exists 1. simpl. rewrite H. reflexivity.
  - destruct IHHce1. destruct IHHce2. exists (S(x + x0)). simpl. rewrite H.
    assert (x <= x + x0) by (apply le_plus_l). 
    assert (ceval_step st c (x + x0) = Some st') by (apply (ceval_step_more x (x + x0) st st' c H2 H0)). 
    rewrite H3. 
    assert (x0 <= x + x0) by (apply le_plus_r). 
    apply (ceval_step_more x0 (x + x0) st' st'' (WHILE b DO c END) H4 H1).
Qed.
(** [] *)
