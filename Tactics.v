...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
intros. apply trans_eq with (minustwo o). rewrite <- H. apply H0. reflexivity.
Qed.
(** [] *)
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
intros. injection H0. intros. symmetry. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m. destruct m.
    ...
    + intros. simpl in H. injection H as H1. rewrite <- plus_n_Sm in H1. rewrite <- plus_n_Sm in H1. injection H1 as H. apply IHn' in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
- intros. destruct n.
  + discriminate H. 
  + simpl in H. injection H as H. apply IHl in H. simpl. apply H.
Qed.
(** [] *)
...
- intros. simpl in H. injection H as H. rewrite <- H. rewrite <- H0. reflexivity.
- intros. destruct x as [m n] eqn:Ex. destruct (split l) as [lx ly] eqn:El. simpl in H. rewrite El in H. injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl. assert (H3:combine lx ly = l). { apply (IHl lx ly). reflexivity. } rewrite H3. reflexivity.
Qed.
(** [] *)
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
intros. induction (f true) eqn:E1. 
- induction (f false) eqn:E2.
  + destruct b.
...
- induction m.
  + intros. discriminate H. 
  + intros. simpl in H. induction p.
    * discriminate H0.
    * simpl. simpl in H0. apply (IHn m p H H0).
Qed.
(** [] *)
...
Proof.
unfold split_combine_statement. intros X Y. induction l1.
- intros. simpl in H. destruct l2.
  + simpl. reflexivity.
  + simpl in H. discriminate H. 
- intros. destruct l2.
  + simpl in H. discriminate H. 
  + simpl in H. injection H as H1. simpl. rewrite (IHl1 l2 H1). reflexivity.
Qed.
...
- simpl in H. destruct (test x0) eqn:E.
  + injection H as H1. rewrite <- H1. apply E.
  + apply (IHl H).
Qed.
(** [] *)
...
- destruct (test x) eqn:E.
  ...
  + unfold existsb'. simpl.  rewrite E.  simpl. rewrite IHl. unfold existsb'. reflexivity.
Qed.
