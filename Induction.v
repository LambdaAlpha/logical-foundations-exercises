...
- induction n as [|n'].
  + simpl. rewrite IHm'. reflexivity.
  + simpl. rewrite IHm'. rewrite <- IHn'.  simpl. rewrite IHm'.
...
- simpl. reflexivity.
- simpl. rewrite plus_comm. simpl. rewrite plus_comm. rewrite IHm. reflexivity.
Qed.
(** [] *)
...
Proof.
intros b. 
assert (H: forall a:nat, S a = a + 1). { intros a. rewrite plus_comm. reflexivity. }
induction b as [|b1|b2].
- simpl. reflexivity.
- simpl. rewrite <- H. reflexivity.
- simpl. rewrite <- IHb2. simpl. rewrite <- plus_assoc. rewrite <- H. reflexivity.
Qed.
...
induction n.
- reflexivity.
- simpl. rewrite plus_comm. simpl. simpl in IHn. rewrite plus_comm in IHn. simpl in IHn. rewrite IHn. simpl. reflexivity.
Qed.

Theorem bin_nat_bin_norm: forall b:bin,
  nat_to_bin (bin_to_nat b) = normalize b.
Proof.
induction b.
- reflexivity.
- destruct (bin_to_nat b) eqn:E.
  + simpl. rewrite E. simpl. simpl in IHb. rewrite <- IHb. reflexivity.
  + simpl. rewrite E. rewrite nat_to_bin_double. induction (nat_to_bin n) eqn:E1.
    * simpl in IHb. rewrite E1 in IHb. simpl in IHb. simpl. rewrite E1. rewrite <- IHb. simpl.  reflexivity.
    * simpl in IHb. rewrite E1 in IHb. simpl in IHb. simpl. rewrite E1. rewrite <- IHb. simpl. reflexivity.
    * simpl in IHb. rewrite E1 in IHb. simpl in IHb. rewrite <- IHb. simpl. rewrite E1. simpl. reflexivity.
- simpl. assert (H: bin_to_nat b + bin_to_nat b + 1 = S (bin_to_nat b + bin_to_nat b)).
  + rewrite <- plus_assoc. rewrite plus_comm. rewrite <- plus_assoc. rewrite plus_comm. simpl. reflexivity.
  + rewrite H. simpl. induction (bin_to_nat b) eqn:E.
    * simpl in IHb. rewrite <- IHb. simpl. reflexivity.
    * rewrite nat_to_bin_double. rewrite IHb. simpl. reflexivity.
Qed.
